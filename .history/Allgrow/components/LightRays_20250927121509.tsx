'use client'; // ensure this is client-side only if using Next.js

import { useRef, useEffect, useState } from 'react';
import type { Renderer , Mesh}
...
const meshRef = useRef<Mesh | null>(null);
  ...
  // WebGL Initialization (client-side only)
  useEffect(() => {
  ...
  // Update uniforms dynamically if props change
  useEffect(() => {
    ...
    if (containerRef.current) {
      const w = containerRef.current.clientWidth * rendererRef.current.dpr;
      const h = containerRef.current.clientHeight * rendererRef.current.dpr;
      const { anchor, dir } = getAnchorAndDir(raysOrigin, w, h);
